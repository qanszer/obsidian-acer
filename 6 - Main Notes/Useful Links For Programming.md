
2025-09-20  09:32

Tags: [[Links]] [[Coding]]

---

# Useful Links For Programming


## Learning

#### 1. Devdocs
https://devdocs.io/
All-in-one documentation for all programming languages and frameworks. Works offline as well when the website is loaded
#### 2. The Odin Project
https://www.theodinproject.com/
Roadmap for becoming a full-stack web developer
#### 3. Onelinerhub
https://onelinerhub.com/
10,000+ Explained code solutions



## Database

#### 1. DBdiagram
https://dbdiagram.io/home
Tool to automatically create an ER diagram just by writing SQL



## Cloud

#### 1. AWS Console
https://aws.amazon.com/
Free AWS Management Console account from CloudNine Worshop
Account ID: keitodanieru
Username: cloud9_attendee_workshop
Password: lets-Go!ADU15



## Github

#### 1. Prettier Extension
https://dev.to/hannahgooding/vs-code-shortcuts-and-tricks-that-i-wish-i-knew-sooner-3mcj#:~:text=relevant%20commit%20message.-,Prettier,-Prettier%20is%20a
For consistent code format on group projects



## Linux

#### 1. Collection of plymouth boot animations
https://github.com/adi1090x/plymouth-themes?tab=readme-ov-file
https://xdaforums.com/t/bootanimations-collection.3721978/
https://drive.google.com/drive/u/0/folders/1UBB1d4qRW_s88oH4-klt7f2JBQh39akL?sort=15&direction=d



## Web Development

#### 1. MendRenovate
https://www.mend.io/renovate-lp/?utm_origin=3rd&utm_from=youtube
For auto-updates on dependencies. Useful for avoiding random errors due to new updates
#### 2. Graphonline
https://graphonline.top/
Tool for creating graphs and then finding the shortest path algorithm
#### 3. Formspree 
https://formspree.io/forms
Tool to connect your website's email form to actually send the message to the designated email account



## Job Related

#### 1. Dev Dispatch
https://dev-dispatch.com/
LinkedIn for programmers. Programming job roles get posted here first before it gets posted on LinkedIn
#### 2. Tech Interview
https://github.com/yangshun/tech-interview-handbook
Useful for tech interviews


## Design

#### 1. Pacdora
https://www.pacdora.com/
3d mockups for many items like a box, shirt, bag, bottle, etc
#### 2. Httpster
https://httpster.net/
Collection of clean website designs
#### 3. Websitevice
https://websitevice.com/
Collection of clean website designs
#### 4. Remove Photos
https://remove.photos/
Tool to remove background to make transparent png images
#### 5. Tabler
https://tabler.io/icons
Collection of free svg icons with customizable color for all
#### 6. Freesvgicons
https://freesvgicons.com/
Collection of free svg icons



